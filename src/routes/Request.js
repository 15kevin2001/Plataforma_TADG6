...
//post para postular a un puesto
router.post('/request', async(req, res) => {
    try {
        ...
        const nuevoRequest = new requestSchema(req.body);

        // Guardar el request en la base de datos
        const requestGuardado = await nuevoRequest.save();

        // Envía una respuesta exitosa
        res.json({ message: `Postulación exitosa al puesto ${position._nombre} del proyecto ${project._titulo}` });
    } catch (error) {
        console.error(error);
...
router.get('/request_byProject', async(req, res) => {
    ...
    try {
        ...
        // Busca todas las solicitudes para el proyecto especificado
        const request = await requestSchema.find({ "_codigo_proyecto": id_project });
